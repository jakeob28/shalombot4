...
impl EventHandler for Handler {
    async fn reaction_add(&self, ctx: Context, add_reaction: Reaction) {
        if add_reaction.user_id.is_some_and(|id| id.eq(&ctx.cache.current_user().id)) {
            return;
        }
        ...
        if !add_reaction.member.as_ref().is_some_and(|member| member.roles.contains(&RoleId::new(BotConfig::global_cfg().guild_settings.support_team_role))) {
            return;
        }

        if add_reaction.message(&ctx.http).await.is_ok_and(|msg| {
            msg.content.starts_with("Thank you for your ticket with Shalom Support") &&
            ...
            while let Some(message_result) = messages.next().await {
                match message_result {
                    Ok(message) => {
                        message_vec.insert(0, format!("[{}] {}: {}", message.timestamp.to_string(), message.author.name, message.content))
                    }
                    Err(_) => {}
                }
            }

            ChannelId::new(BotConfig::global_cfg().guild_settings.ticket_log_channel).send_message(
                &ctx.http, CreateMessage::new().content("Ticket Logged")
                    .add_file(CreateAttachment::bytes(message_vec.join("\n"), "log.txt")),
            ).await.unwrap();

            match add_reaction.channel_id.delete(&ctx.http).await {
                Ok(_) => { info!("Ticket Channel Deleted")}
                Err(error) => { error!("Error Deleting Ticket Channel: {}", error)}
            }
        };
    }
    ...
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        let commands = GuildId::set_commands(guild_id, &ctx.http, vec![
    ...
    async fn interaction_create(&self, ctx: Context, interaction: Interaction) {
        if let Interaction::Command(command) = interaction {
            ...
            if let Err(why) = command
                .create_response(&ctx.http, CreateInteractionResponse::Message(CreateInteractionResponseMessage::new().content(content)))
                .await
            {
