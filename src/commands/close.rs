...
pub async fn run(ctx: &Context, command: &CommandInteraction) -> String {
    if !command.member.as_ref()
        .is_some_and(|m| m.roles.contains(&RoleId::new(BotConfig::global_cfg().guild_settings.support_team_role))) {
        return "This command is restricted to the Shalom Support team!".to_string();
    }

    if !command.channel_id.name(&ctx.http).await.unwrap_or("".to_string()).starts_with("ticket") {
    ...
    msg.react(&ctx.http, ReactionType::from('❌')).await.expect("error reacting");
...
pub fn register() -> CreateCommand {
    CreateCommand::new("close")
        .description("Close a shalom ticket")
}